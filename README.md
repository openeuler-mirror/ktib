...
- 镜像分析：`ktib analyze` 分析镜像空间浪费、扫描软件包（含 Hash 校验）并提供优化建议
- 镜像融合：`ktib fusion` 基于依赖分析与 RPM 数据库重构，将臃肿镜像裁剪为精简镜像
