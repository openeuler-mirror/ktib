...
- `--locale`：默认配置中的语言（可选，默认：`en_US.UTF-8`）
- `<项目路径>`：要初始化项目的目录路径（必需）
...
# 指定类型并写入默认配置
ktib project init --type init --config /path/to/project/config.yml --timezone "Asia/Shanghai" --locale "en_US.UTF-8" /path/to/project
```
